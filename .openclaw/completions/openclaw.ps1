...
Register-ArgumentCompleter -Native -CommandName openclaw -ScriptBlock {
    ...
    if ($commandPath -eq "") {
         ...
         $completions | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
            ...
            if ($commandPath -eq 'openclaw heartbeat-v2') {
                $completions = @('status','trigger','pause','resume','analytics')
                $completions | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
                    [System.Management.Automation.CompletionResult]::new($_, $_, 'ParameterName', $_)
                }
            }

            if ($commandPath -eq 'openclaw heartbeat-v2 status') {
                $completions = @('-a')
                $completions | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
                    [System.Management.Automation.CompletionResult]::new($_, $_, 'ParameterName', $_)
                }
            ...
            if ($commandPath -eq 'openclaw heartbeat-v2 trigger') {
                $completions = @('-r')
                $completions | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
                    [System.Management.Automation.CompletionResult]::new($_, $_, 'ParameterName', $_)
                }
            ...
            if ($commandPath -eq 'openclaw heartbeat-v2 pause') {
                $completions = @('-r')
                $completions | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
                    [System.Management.Automation.CompletionResult]::new($_, $_, 'ParameterName', $_)
                }
            ...
            if ($commandPath -eq 'openclaw heartbeat-v2 analytics') {
                $completions = @('-t')
                $completions | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
                    [System.Management.Automation.CompletionResult]::new($_, $_, 'ParameterName', $_)
                }
