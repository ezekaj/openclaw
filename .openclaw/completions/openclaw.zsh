  ...
  case $state in
    (args)
      case $line[1] in
        ...
        (heartbeat-v2) _openclaw_heartbeat_v2 ;;
...
_openclaw_heartbeat_v2_status() {
  _arguments -C \
    "(--agent -a)"{--agent,-a}"[Agent ID to check]"
}

_openclaw_heartbeat_v2_trigger() {
  _arguments -C \
    "(--reason -r)"{--reason,-r}"[Reason for trigger]"
}

_openclaw_heartbeat_v2_pause() {
  _arguments -C \
    "(--reason -r)"{--reason,-r}"[Reason for pause]"
}

_openclaw_heartbeat_v2_resume() {
  _arguments -C \
    
}

_openclaw_heartbeat_v2_analytics() {
  _arguments -C \
    "(--time-range -t)"{--time-range,-t}"[Time range (1h, 24h, 7d, 30d)]"
}

_openclaw_heartbeat_v2() {
  local -a commands
  local -a options
  
  _arguments -C \
     \
    "1: :_values 'command' 'status[Show heartbeat system status]' 'trigger[Trigger immediate heartbeat for an agent]' 'pause[Pause heartbeats for an agent]' 'resume[Resume heartbeats for an agent]' 'analytics[Show analytics for an agent]'" \
    "*::arg:->args"

  case $state in
    (args)
      case $line[1] in
        (status) _openclaw_heartbeat_v2_status ;;
        (trigger) _openclaw_heartbeat_v2_trigger ;;
        (pause) _openclaw_heartbeat_v2_pause ;;
        (resume) _openclaw_heartbeat_v2_resume ;;
        (analytics) _openclaw_heartbeat_v2_analytics ;;
      esac
      ;;
  esac
}
