 ...
 */
export function runMigrations(db: Database.Database): SchemaVersion {
 ...
 */
export function populateFtsIndex(db: Database.Database): number {
 ...
 */
export function checkFtsIntegrity(db: Database.Database): boolean {
  try {
    const result = db
 ...
 */
export function backfillCreatedAt(db: Database.Database): number {
 ...
 */
export function updateChunkAccess(
 ...
 */
export function batchUpdateAccess(
  db: Database.Database,
  chunkIds: string[]
): void {
  const now = Date.now();
 ...
 */
export function getCachedRerankScore(
  db: Database.Database,
  queryHash: string,
  chunkId: string,
  ...
  const result = db
    ...
    `)
    .get(queryHash, chunkId, cutoff) as { score: number } | undefined;
 ...
 */
export function cacheRerankScore(
  db: Database.Database,
  queryHash: string,
  chunkId: string,
  score: number,
 ...
 */
export function batchCacheRerankScores(
  db: Database.Database,
  queryHash: string,
 ...
 */
export function cleanRerankCache(
  db: Database.Database,
...
export function getSearchStats(
  db: Database.Database,
  sinceMs: number = 86400000 // 24 hours
): {
  ...
  const totals = db
    .prepare(`
      ...
      WHERE created_at > ?
    `)
    .get(cutoff) as { total: number; avg_duration: number; avg_results: number };
  ...
  const byType = db
    ...
    `)
    .all(cutoff) as Array<{ search_type: string; count: number }>;
 ...
 */
export function pruneAccessLogs(
  db: Database.Database,
  maxAgeMs: number = 604800000 // 7 days
): number {
  const cutoff = Date.now() - maxAgeMs;
...
export function getDatabaseStats(db: Database.Database): {
  ...
  try {
    rerankCacheCount = (
      db.prepare("SELECT COUNT(*) as count FROM rerank_cache").get() as { count: number }
    ).count;
  } catch {
  ...
  try {
    accessLogCount = (
      db.prepare("SELECT COUNT(*) as count FROM memory_access_log").get() as { count: number }
    ).count;
  } catch {
 ...
 */
export function verifyFtsSync(db: Database.Database): {
  inSync: boolean;
  chunkCount: number;
  ftsCount: number;
  missing: number;
