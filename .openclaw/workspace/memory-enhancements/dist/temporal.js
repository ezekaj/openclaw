 ...
 */
export function getDecayParams(config) {
 ...
 */
export function calculateAgeDays(documentTimestamp, referenceDate) {
 ...
 */
export function calculateDecayFactor(ageDays, lambda, minWeight) {
    if (lambda === 0) {
 ...
 */
export function calculateTemporalWeight(documentTimestamp, config = DEFAULT_TEMPORAL_CONFIG) {
    if (!config.enabled) {
        return 1.0;
    }
 ...
 */
export function applyTemporalWeighting(results, config = DEFAULT_TEMPORAL_CONFIG) {
    if (!config.enabled) {
        return results;
    }
 ...
 */
export function applyTemporalWeightingAndSort(results, config = DEFAULT_TEMPORAL_CONFIG) {
 ...
 */
export function describeDecayProfile(profile) {
 ...
 */
export function calculateWeightAtAge(ageDays, config = DEFAULT_TEMPORAL_CONFIG) {
    if (!config.enabled) {
        return 1.0;
    }
    const { lambda } = getDecayParams(config);
 ...
 */
export function generateDecayCurve(config = DEFAULT_TEMPORAL_CONFIG, maxDays = 365) {
    const points = [];
    const { lambda } = getDecayParams(config);
 ...
 */
export function estimateAgeAtWeight(targetWeight, config = DEFAULT_TEMPORAL_CONFIG) {
    if (!config.enabled) {
 ...
 */
export function createCustomTemporalConfig(halfLifeDays, minWeight = 0.3) {
    return {
        enabled: true,
 ...
 */
export function batchCalculateWeights(timestamps, config = DEFAULT_TEMPORAL_CONFIG) {
 ...
 */
export function getTemporalStats(results, config = DEFAULT_TEMPORAL_CONFIG) {
    if (results.length === 0) {
        return {
            oldestDays: 0,
            newestDays: 0,
