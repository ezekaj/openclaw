 ...
 */
export function searchBM25(db, query, limit, sources) {
  ...
  `;
    try {
        ...
        const results = stmt.all(ftsQuery, ...sourceParams, limit);
        return results;
    }
    catch (error) {
 ...
 */
export function getBM25Score(db, query, chunkId) {
    const ftsQuery = buildFtsQuery(query);
    if (!ftsQuery) {
        return null;
    }
  ...
  `;
    try {
        const stmt = db.prepare(sql);
        const result = stmt.get(ftsQuery, chunkId);
 ...
 */
export function checkFtsIndex(db) {
    try {
 ...
 */
export function getTermStats(db, query) {
    const tokens = query
        .toLowerCase()
        .split(/[^a-z0-9]+/)
        .filter((t) => t.length > 0);
