 ...
 */
function getCacheKey(query, passageId, model) {
    return `${model}:${query.slice(0, 100)}:${passageId}`;
}
/**
 * Get cached score if available and not expired
 */
function getCachedScore(query, passageId, model) {
 ...
 */
function setCachedScore(query, passageId, model, score) {
    const key = getCacheKey(query, passageId, model);
    rerankCache.set(key, {
        score,
        model,
 ...
 */
async function rerankWithLLM(query, passages, config) {
 ...
 */
async function batchRerankWithLLM(query, passages, config) {
    const endpoint = config.apiEndpoint || DEFAULT_LM_STUDIO_ENDPOINT;
    const model = config.model || "qwen2.5-coder-1.5b-instruct";
    const timeout = config.timeoutMs || DEFAULT_TIMEOUT_MS;
 ...
 */
async function rerankWithCohere(query, passages, config) {
    if (!config.apiKey) {
 ...
 */
async function rerankWithLocal(_query, passages, _config) {
 ...
 */
function getRerankFunction(provider) {
    switch (provider) {
        case "llm":
 ...
 */
export async function rerank(query, results, config = DEFAULT_RERANK_CONFIG) {
    ...
    const cachedScores = candidates.map((r) => getCachedScore(query, r.id, model));
    const allCached = cachedScores.every((s) => s !== null);
    let rerankScores;
    if (allCached) {
        // All scores cached
        rerankScores = cachedScores;
    }
    else {
 ...
 */
export async function rerankWithBlending(query, results, config = DEFAULT_RERANK_CONFIG, originalWeight = 0.3, rerankWeight = 0.7) {
 ...
 */
export async function checkRerankService(config = DEFAULT_RERANK_CONFIG) {
    try {
 ...
 */
export function createRerankConfig(provider, options) {
    const defaults = {
        llm: {
 ...
 */
export async function scorePassage(query, passage, config = DEFAULT_RERANK_CONFIG) {
    const response = await rerank(query, [
        {
            id: "test",
            path: "",
            source: "",
